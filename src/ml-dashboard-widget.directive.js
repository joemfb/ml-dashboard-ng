(function () {
  ...
  angular.module('ml.dashboard')
    .directive('mlDashboardWidget', mlDashboardWidget)
    .controller('MlDashboardWidgetCtrl', MlDashboardWidgetCtrl)
    .directive('mlDashboardCollapsibleSection', CollapsibleSectionDirective);

  function mlDashboardWidget() {
    return {
      restrict: 'A',
      scope: {
      ...
      controller: 'MlDashboardWidgetCtrl',
      templateUrl: '/templates/ml-dashboard-widget.html',
  ...
  MlDashboardWidgetCtrl.$inject = ['$scope', 'MLDashboardService'];

  function MlDashboardWidgetCtrl($scope, payorService) {
    var model = {
      sizes: ['Full', 'Half'],
      resultLimit: 100,
